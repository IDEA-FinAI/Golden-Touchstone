...
class RE_en_eval:
    ...
    def get_result(self,file_path,output_path):
        ...
        task = file_path.split('/')[-1].split('_predictions.jsonl')[0]
        with open(file_path,'r',encoding='utf-8') as f:
            for line in f.readlines():
                content = json.loads(line)
                # if content['label'] == preprocess_output(content['predict']):
                tp,fp,fn = self.cpt_label_and_pred(content['predict'],content['label'],content['prompt'][1]['content'])
                tp_all += tp
                fp_all += fp
                fn_all += fn
...
class Ner_en_eval:
    ...
    def get_result(self, file_path, output_path):
        ...
        with open('/finance_ML/FinLLM/FinLLM-benchmark/EN-benchmark/flare-ner/flare-ner_test.json','r') as f:
            lines = json.load(f)
            for line in lines:
...
eval_dataset_path = f"/finance_ML/FinLLM/FinLLM-benchmark/gpt-4o-prediction"

output_dir = f"/finance_ML/FinLLM/sh_test/eval/{Model}"
...
count = 0
result = 0
for file in tqdm(os.listdir(eval_dataset_path)):
    #判断是否为文件，如果是就去拿llama-factory的generated_predictions.jsonl
    file_path = os.path.join(eval_dataset_path,file)
    if os.path.isfile(file_path):
        name = file.split('_predictions.jsonl')[0]
        if name in class_enabel.keys():
            classchoice = class_enabel[name]
            evalclass = classchoice()
            result += evalclass.get_result(file_path,output_dir)
            count += 1
